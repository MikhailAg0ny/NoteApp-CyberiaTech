...
## 2. Building the Blaze transaction with metadata
When the UI needs to persist a note, `submitNoteTransaction` performs:
...
- The note list/modal (`page.tsx`, `NoteModal.tsx`) now show the tx status (Pending/Confirmed) and short tx hash for context.
- Since the row already contains `tx_hash` and `tx_status='pending'`, users see the note as soon as metadata submission succeeds.
